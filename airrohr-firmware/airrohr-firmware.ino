...
bool is_offline = OFFLINE;
bool has_sdcard = HAS_SD;
int  debug = 5;
...
unsigned long sending_intervall_ms = 5000;
...
String SDS_version_date() {
  ...
  while (serialSDS.available() > 0) {
    ...
    switch (len) {
      ...
      case (6): if (value < 0x10) {
          device_id  = "0" + String(value, HEX);
        } else {
          device_id  = String(value, HEX);
        }; checksum_is += value; break;
      case (7): if (value < 0x10) {
          device_id += "0";
        }; device_id += String(value, HEX); checksum_is += value; break;
      case (8):
...
/*****************************************************************
  ...
  /*****************************************************************/
void webserver_root() {
  if (WiFi.status() != WL_CONNECTED && !is_offline) {
...
/*****************************************************************
  /* Webserver sdcard: Show options and graphs for sd card data    *
  /*****************************************************************/
void webserver_sdcard() {
  webserver_request_auth();
...
void setup_webserver() {
  ...
  server.begin();
  debug_out(F("Webserver gestartet... "), DEBUG_MIN_INFO, 1);
}
/*****************************************************************
  /* WifiConfig                                                    *
  /*****************************************************************/
...
void localWiFi() {
  ...
  debug_out(F("Starting Local WiFiManager"), DEBUG_MIN_INFO, 1);
...
void connectWifi() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    debug_out("AP ID: Feinstaubsensor-" + esp_chipid + " - IP: 192.168.4.1", DEBUG_MIN_INFO, 1);
    if (!is_offline)
      wifiConfig();
    else
      localWiFi();
...
void send_to_sdcard() {
  ...
  if (HAS_SD != has_sdcard) {
    display_debug(F("Cant write to SDCARD"));
    debug_out(F("Cant write to SDCARD"), DEBUG_ERROR, 1);
    return;
  }
  ...
  if (long(act_milli - starttime) < (long(sending_intervall_ms) - long(warmup_time_SDS_ms + reading_time_SDS_ms))) {
    ...
    while (serialSDS.available() > 0) {
      ...
      switch (len) {
        ...
        case (22): if (msg_len == 24) {
            checksum_should = ( value << 8 );
          } else {
            checksum_is += value;
          }; break;
        case (23): if (msg_len == 24) {
...
void setup() {
  Wire.begin(D3, D4);
  Serial.begin(19200);					// Output to Serial at 9600 baud
  while (!Serial) {
  ...
  debug_out(F("Check Autoupdate"), DEBUG_MIN_INFO, 1);
  if (!is_offline) {
    autoUpdate();
  }
...
#endif
  debug_out(F("Aktive Module: "), DEBUG_MIN_INFO, 1);
  if (ppd_read) {
  ...
  if (pms24_read) {
    debug_out(F("Lese PMS3003..."), DEBUG_MIN_INFO, 1);
  }
  if (pms32_read) {
    debug_out(F("Lese 1003 or 7003..."), DEBUG_MIN_INFO, 1);
  }
  if (dht_read) {
    debug_out(F("Lese DHT..."), DEBUG_MIN_INFO, 1);
  }
  if (htu21d_read) {
    debug_out(F("Lese HTU21D..."), DEBUG_MIN_INFO, 1);
  }
  if (bmp_read) {
    debug_out(F("Lese BMP..."), DEBUG_MIN_INFO, 1);
  }
  if (bmp280_read) {
    debug_out(F("Lese BMP280..."), DEBUG_MIN_INFO, 1);
  }
  if (bme280_read) {
    debug_out(F("Lese BME280..."), DEBUG_MIN_INFO, 1);
  }
  if (ds18b20_read) {
    debug_out(F("Lese DS18B20..."), DEBUG_MIN_INFO, 1);
  }
  if (gps_read) {
    debug_out(F("Lese GPS..."), DEBUG_MIN_INFO, 1);
  }
  if (send2dusti) {
    debug_out(F("Sende an luftdaten.info..."), DEBUG_MIN_INFO, 1);
  }
  if (send2madavi) {
    debug_out(F("Sende an madavi.de..."), DEBUG_MIN_INFO, 1);
  }
  if (send2lora) {
    debug_out(F("Sende an LoRa gateway..."), DEBUG_MIN_INFO, 1);
  }
  if (send2csv) {
    debug_out(F("Sende als CSV an Serial..."), DEBUG_MIN_INFO, 1);
  }
  if (send2custom) {
    debug_out(F("Sende an custom API..."), DEBUG_MIN_INFO, 1);
  }
  if (send2influx) {
    debug_out(F("Sende an custom influx DB..."), DEBUG_MIN_INFO, 1);
  }
  if (auto_update) {
    debug_out(F("Auto-Update wird ausgeführt..."), DEBUG_MIN_INFO, 1);
  }
  if (has_display) {
    debug_out(F("Zeige auf Display..."), DEBUG_MIN_INFO, 1);
  }
  if (has_lcd1602) {
    debug_out(F("Zeige auf LCD 1602..."), DEBUG_MIN_INFO, 1);
  }
  if (bmp_read) {
    if (!bmp.begin()) {
...
void loop() {
  ...
  if (send_now) {
    ...
    if (has_sdcard) {
      debug_out(F("## Sending to sdcard: "), DEBUG_MIN_INFO, 1);
      send_to_sdcard();
    }
